...
describe('MessageInput', () => {
  ...
  describe('rendering', () => {
    ...
    it('should render attachment button when enabled', () => {
      ...
      const fileInput = screen.getByRole('textbox').parentElement?.parentElement?.querySelector(
        'input[type="file"]'
      );
  ...
  describe('attachments', () => {
    it('should not show attachment preview when none added', () => {
      ...
      const { container } = render(<MessageInput onSend={mockOnSend} enableAttachments={true} />);
      expect(container.querySelector('.bg-neutral-100.rounded-md')).not.toBeInTheDocument();
    });
    ...
    it('should accept multiple file types', () => {
      ...
      const fileInput = document.querySelector('input[type="file"]') as HTMLInputElement;
      expect(fileInput).toHaveAttribute('accept', '.pdf,.png,.jpg,.jpeg,.mp3,.wav,.txt,.md');
    });

    it('should disable attachment button when disabled', () => {
      render(<MessageInput onSend={mockOnSend} enableAttachments={true} disabled={true} />);
  ...
  describe('accessibility', () => {
    ...
    it('should be keyboard navigable', async () => {
      ...
      // Tab again should focus send button
      await user.tab();
      const sendButton = screen.getByRole('button', { name: /send message/i });
