...
describe('MessageList', () => {
  ...
  describe('empty state', () => {
    ...
    it('should not render virtuoso when empty', () => {
      ...
      expect(container.querySelector('[data-testid="virtuoso-list"]')).not.toBeInTheDocument();
    });
  });
  ...
  describe('message rendering', () => {
    it('should render all messages in virtuoso', () => {
      ...
      expect(screen.getByText('Tell me about treatment protocols.')).toBeInTheDocument();
    });
    ...
    it('should pass correct props to MessageBubble', () => {
      ...
      const secondMessage = container.querySelector('[data-message-id="msg-2"]');
      expect(secondMessage?.querySelector('.justify-start')).toBeInTheDocument();
    });
  ...
  describe('typing indicator', () => {
    ...
    it('should not show footer typing indicator when streaming a message', () => {
      const { container } = render(
        <MessageList messages={mockMessages} isTyping={true} streamingMessageId="msg-2" />
      );
  ...
  describe('streaming state', () => {
    it('should pass isStreaming to correct message bubble', () => {
      ...
      const streamingMessage = container.querySelector('[data-message-id="msg-2"]');
    ...
    it('should not show streaming when streamingMessageId does not match', () => {
      const { container } = render(
        <MessageList messages={mockMessages} isTyping={true} streamingMessageId="non-existent" />
      );

      // No message should show streaming indicator
      mockMessages.forEach((msg) => {
        const message = container.querySelector(`[data-message-id="${msg.id}"]`);
    ...
    it('should handle streaming when isTyping is false', () => {
      const { container } = render(
        <MessageList messages={mockMessages} isTyping={false} streamingMessageId="msg-2" />
      );
  ...
  describe('long message lists', () => {
    it('should render large number of messages efficiently', () => {
      ...
      expect(container.querySelector('[data-testid="virtuoso-list"]')).toBeInTheDocument();
