...
export function CollapsibleSidebar({
  ...
  const {
    ...
    prefetchConversationMessages = () => Promise.resolve(),
  } = useConversations({
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    ...
    if (
      window.confirm(
        "This will permanently delete this conversation and all of its messages. Are you sure you want to continue?",
      )
    ) {
      try {
      ...
      >
        ...
        {isLoading && conversations.length === 0 && (
          <div className="px-3 py-2 space-y-2">
            {/* Render 6 skeleton items with alternating badges */}
            {[...Array(6)].map((_, index) => (
              <ConversationItemSkeleton key={index} showBadge={index % 2 === 0} />
            ))}
          </div>
        )}
            ...
            <ul className="space-y-1">
              {pinnedConversations.map((conv) => (
                <ConversationItem
                  ...
                  onHover={() =>
                    prefetchConversationMessages(conv.id).catch(() => {})
                  }
...
function ConversationItem({
  ...
  const phiMode = (conversation as any).phiMode as
    | "clinical"
    | "demo"
    | undefined;
  const tags = ((conversation as any).tags as string[] | undefined) || [];
          ...
          {tags.length > 0 && (
            <p className="mt-0.5 flex flex-wrap gap-1">
              {tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-neutral-100 text-[10px] text-neutral-600"
                >
                  {tag}
                </span>
              ))}
...
function ConversationItemSkeleton({ showBadge = false }: { showBadge?: boolean }) {
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg animate-pulse">
      {/* Icon placeholder */}
      <div className="w-4 h-4 bg-neutral-200 rounded flex-shrink-0" />

      {/* Content */}
      <div className="flex-1 min-w-0 space-y-2">
        {/* Title skeleton */}
        <div className="h-4 bg-neutral-200 rounded w-3/4" />
        {/* Meta skeleton (time + optional badge) */}
        <div className="flex items-center gap-2">
          <div className="h-3 bg-neutral-100 rounded w-12" />
          {/* PHI badge skeleton (shown on alternating items) */}
          {showBadge && (
            <div className="h-4 bg-neutral-100 rounded-full w-12" />
          )}
        </div>
      </div>
    </div>
