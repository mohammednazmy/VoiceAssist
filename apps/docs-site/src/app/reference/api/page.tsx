...
function MethodBadge({ method }: { method: string }) {
  const styles = METHOD_STYLES[method] || "bg-slate-100 text-slate-800 border-slate-200";
  return (
    <span
...
function buildOperationsByTag(spec: GeneratedApiSpec): { groups: [string, GeneratedOperation[]][]; tagLookup: Map<string, string | undefined> } {
  ...
  const sortedGroups = Object.entries(operationsByTag).sort(([tagA], [tagB]) => {
    ...
    <div className="space-y-10">
      <div className="mb-6 space-y-3">
        ...
        <p className="text-lg text-gray-600 dark:text-gray-400">
          Complete reference for REST endpoints, WebSocket events, and generated OpenAPI documentation for VoiceAssist.
        </p>
      </div>

      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Spec metadata</h3>
          <dl className="mt-2 space-y-1 text-sm text-slate-600 dark:text-slate-400">
            <div className="flex items-center justify-between">
              <dt>Title</dt>
              <dd className="font-medium text-slate-900 dark:text-slate-100">{spec.info?.title || "API"}</dd>
            </div>
            <div className="flex items-center justify-between">
              <dt>Version</dt>
              <dd className="font-medium text-slate-900 dark:text-slate-100">{spec.info?.version || "N/A"}</dd>
            </div>
            {spec.info?.description && <p className="pt-2 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{spec.info.description}</p>}
          </dl>
        </div>

        <div className="rounded-lg border border-blue-200 bg-blue-50 p-4 shadow-sm dark:border-blue-900/70 dark:bg-blue-900/20">
          <h3 className="text-sm font-semibold text-blue-900 dark:text-blue-100">Servers</h3>
          <ul className="mt-2 space-y-1 text-sm text-blue-800 dark:text-blue-100">
            {(spec.servers || []).map((server) => (
              <li key={server.url} className="flex flex-col rounded border border-blue-200/60 bg-white/70 px-3 py-2 dark:border-blue-800 dark:bg-blue-900/40">
                <code className="font-mono text-xs">{server.url}</code>
                {server.description && <span className="text-[11px] text-blue-700 dark:text-blue-200">{server.description}</span>}
              </li>
            ))}
            {spec.servers?.length === 0 && <li className="text-xs text-blue-700 dark:text-blue-200">No servers defined in the OpenAPI spec.</li>}
          </ul>
        </div>
        ...
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 shadow-sm dark:border-emerald-900/70 dark:bg-emerald-900/20">
          <h3 className="text-sm font-semibold text-emerald-900 dark:text-emerald-100">Helpful links</h3>
      ...
      <div className="space-y-8">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900 dark:text-white">Generated operations</h2>
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Operations are generated from the OpenAPI spec during CI to keep this page in sync with the backend implementation.
          </p>
        </div>

        <div className="space-y-6">
          {groups.map(([tag, operations]) => (
            <section key={tag} className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{tag}</h3>
                  {tagLookup.get(tag) && (
                    <p className="text-sm text-slate-600 dark:text-slate-400">{tagLookup.get(tag)}</p>
                  )}
                </div>
                ...
                {operations.map((operation) => (
                  ...
                  >
                    ...
                    {operation.description && (
                      <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{operation.description}</p>
                    )}

                    {operation.parameters.length > 0 && (
                      <div className="mt-3 space-y-2">
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Parameters</h4>
                          ...
                          {operation.parameters.map((param) => (
                            ...
                            >
                              <div className="flex flex-wrap items-center gap-2">
                                <span className="font-mono text-xs text-slate-900 dark:text-slate-100">{param.name}</span>
                              ...
                              {param.description && (
                                <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">{param.description}</p>
                              )}
                    ...
                    {operation.requestBody && (
                      <div className="mt-3 space-y-2">
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Request body</h4>
                        <div className="rounded border border-slate-200 bg-white px-3 py-2 text-sm dark:border-slate-800 dark:bg-slate-950/40">
                          ...
                          {operation.requestBody.description && (
                            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">{operation.requestBody.description}</p>
                          )}
                    ...
                    {operation.responses.length > 0 && (
                      <div className="mt-3 space-y-2">
                        <h4 className="text-sm font-semibold text-slate-900 dark:text-slate-100">Responses</h4>
                        <div className="space-y-2">
                            ...
                            >
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {response.description && (
                                  <span className="text-xs text-slate-600 dark:text-slate-400">{response.description}</span>
                                )}
                              </div>
      ...
      <div className="space-y-12">
        {apiDoc && (
          <div className="border-t border-gray-200 pt-8 dark:border-gray-800">
            <h2 className="mb-4 text-2xl font-semibold text-gray-900 dark:text-white">Manual API overview</h2>
        ...
        {endpointsDoc && (
          <div className="border-t border-gray-200 pt-8 dark:border-gray-800">
            <h2 className="mb-4 text-2xl font-semibold text-gray-900 dark:text-white">Endpoints reference</h2>
        ...
        {kbFunctions && (
          <div className="border-t border-gray-200 pt-8 dark:border-gray-800">
            <h2 className="mb-4 text-2xl font-semibold text-gray-900 dark:text-white">KB function reference</h2>
        ...
        {implementationDoc && (
          <div className="border-t border-gray-200 pt-8 dark:border-gray-800">
            <h2 className="mb-4 text-2xl font-semibold text-gray-900 dark:text-white">Client implementation</h2>
