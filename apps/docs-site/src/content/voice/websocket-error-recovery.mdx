...
- WS performance assertions use relative timestamps: barge-in latency must stay under 10s max (avg < 8s) and first WS response under 10s; connection/first-audio checks remain in the low seconds.
- To avoid multi-project misfires, explicitly scope Playwright runs, e.g. `pnpm exec playwright test --project=voice-live e2e/voice/voice-websocket-integration.spec.ts` for live checks, `--project=voice-smoke` for the flag smoke matrix, or `--project=voice-nightly` when invoking the full matrix on CI.
