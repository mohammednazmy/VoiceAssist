...
export function DocPage({ title, description, docPaths }: DocPageProps) {
  const loadedDocs = docPaths
    ...
    .filter((entry) => entry.doc !== null) as { path: string; doc: NonNullable<ReturnType<typeof loadDoc>> }[];

  const combinedContent = loadedDocs.map((entry) => entry.doc.content).join("\n\n---\n\n");

  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-wide text-primary-600 dark:text-primary-400">VoiceAssist Docs</p>
      ...
      {loadedDocs.length > 0 ? (
        ...
        <div className="rounded-lg border border-dashed border-gray-200 bg-gray-50 p-6 text-gray-700 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-200">
          ...
          <p className="mt-2 text-sm">Add one of the following markdown files under docs/ to populate this page:</p>
