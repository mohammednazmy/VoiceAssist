...
export function Dashboard() {
  const { summary: _summary, loading: _loading } = useAdminSummary();
  return (
    <section className="flex-1 p-6 space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500">
          High-level status of the VoiceAssist deployment. Data is partially
          mocked until the full backend is implemented.
        </p>
      </div>
      <div className="grid grid-cols-3 gap-4 text-xs">
        <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3">
          ...
          <div className="mt-1 text-[10px] text-slate-500">
            Aggregate from chat-service metrics
          </div>
        </div>
        <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3">
          ...
          <div className="mt-1 text-[10px] text-slate-500">
            From Prometheus tool_errors_total
          </div>
        </div>
        <div className="bg-slate-950/70 border border-slate-800 rounded-lg p-3">
