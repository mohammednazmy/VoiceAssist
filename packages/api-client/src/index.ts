...
const cryptoApi: CryptoLike | undefined = typeof crypto !== "undefined" ? (crypto as CryptoLike) : undefined;
...
const buildTraceparent = (): string => {
  const traceId = (cryptoApi?.randomUUID?.().replace(/-/g, "") || randomHex(32)).slice(0, 32);
...
export class VoiceAssistApiClient {
  ...
  constructor(config: ApiClientConfig) {
    ...
    this.correlationId = this.config.correlationId || cryptoApi?.randomUUID?.() || `corr-${randomHex(16)}`;
    ...
    this.client.interceptors.request.use((requestConfig) => {
      ...
      if (requestConfig.headers) {
        ...
        requestConfig.headers["traceparent"] = requestConfig.headers["traceparent"] || traceparent;
      }
