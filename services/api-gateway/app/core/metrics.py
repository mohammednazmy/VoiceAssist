...
# Cache Metrics
cache_hits_total = _safe_counter("voiceassist_cache_hits_total", "Total cache hits", ["cache_layer", "cache_key_prefix"])
cache_misses_total = _safe_counter("voiceassist_cache_misses_total", "Total cache misses", ["cache_layer", "cache_key_prefix"])
cache_latency_seconds = _safe_histogram("voiceassist_cache_latency_seconds", "Cache operation latency", ["cache_layer", "operation"])
...
cache_evictions_total = _safe_counter("voiceassist_cache_evictions_total", "Total cache evictions", ["cache_layer", "reason"])

# RAG Query Metrics
rag_query_duration_seconds = _safe_histogram("voiceassist_rag_query_duration_seconds", "RAG query duration", ["stage"])
rag_search_results_total = _safe_histogram("voiceassist_rag_search_results_total", "Search results count", buckets=[0, 1, 2, 3, 5, 10, 20])
...
# RBAC Metrics
rbac_checks_total = _safe_counter("voiceassist_rbac_checks_total", "RBAC checks", ["result", "endpoint", "required_role"])

# API Metrics
http_requests_total = _safe_counter("voiceassist_http_requests_total", "HTTP requests", ["method", "endpoint", "status_code"])
http_request_duration_seconds = _safe_histogram("voiceassist_http_request_duration_seconds", "HTTP duration", ["method", "endpoint"])
websocket_connections_total = _safe_gauge("voiceassist_websocket_connections_total", "Active WebSocket connections")
websocket_messages_total = _safe_counter("voiceassist_websocket_messages_total", "WebSocket messages", ["direction", "message_type"])

# Database Metrics
db_connections_total = _safe_gauge("voiceassist_db_connections_total", "DB connections", ["pool_name", "state"])
db_query_duration_seconds = _safe_histogram("voiceassist_db_query_duration_seconds", "DB query duration", ["query_type"])
...
external_api_requests_total = _safe_counter("voiceassist_external_api_requests_total", "External API requests", ["service", "endpoint", "status_code"])
external_api_duration_seconds = _safe_histogram("voiceassist_external_api_duration_seconds", "External API duration", ["service", "endpoint"])

# Document Processing Metrics
document_processing_jobs_total = _safe_counter("voiceassist_document_processing_jobs_total", "Document processing jobs", ["status"])
document_processing_duration_seconds = _safe_histogram("voiceassist_document_processing_duration_seconds", "Document processing duration", ["file_type"])
...
health_check_duration_seconds = _safe_histogram("voiceassist_health_check_duration_seconds", "Health check duration", ["check_name"])
