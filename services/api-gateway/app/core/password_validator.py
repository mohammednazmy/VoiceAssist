...
class PasswordValidator:
    ...
    def validate(self, password: str) -> Tuple[bool, List[str]]:
        ...
        if (len(password) < 16 and
            self.require_uppercase and self.require_lowercase and
            self.require_digits and self.require_special):
    ...
    def get_strength_score(self, password: str) -> int:
        ...
        if password not in self.COMMON_PASSWORDS: score += 10
        # Extra length bonuses for very long passwords
        if len(password) >= 20: score += 10
        if len(password) >= 28: score += 10
        if len(password) >= 36: score += 10
