...
async def test_clinical_context_with_rag_query(db: Session):
    ...
    saved_context = db.query(ClinicalContext).filter(
        ClinicalContext.user_id == user_id
    ).first()
...
async def test_message_with_attachments_and_citations(db: Session):
    ...
    citations = db.query(MessageCitation).filter(
...
async def test_folder_hierarchy_with_sessions(db: Session):
    ...
    # Create root folder
    root_folder = ConversationFolder(
        id=uuid4(),
        user_id=user_id,
        name="Medical Cases",
        color="#3498db"
    ...
    # Verify hierarchy
    saved_folder = db.query(ConversationFolder).filter(
        ConversationFolder.id == sub_folder.id
    ).first()

    assert saved_folder.parent_folder_id == root_folder.id
    assert saved_folder.name == "Cardiology"

    saved_session = db.query(ChatSession).filter(
